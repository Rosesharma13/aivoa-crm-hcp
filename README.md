...
**Live App:** https://quelvox-crm.vercel.app  
...
The assignment specifies `gemma2-9b-it`. This model was **decommissioned by Groq on June 17, 2026** — any API call to it returns a `model_decommissioned` error. Per Groq's official deprecation guidance, this project uses `llama-3.3-70b-versatile` as the replacement, which is Groq's current recommended general-purpose model. The integration logic, prompt structure, and tool behavior are identical — only the model string changed.

## Note on Database

The assignment specifies MySQL or PostgreSQL. This project uses **SQLite** via SQLAlchemy's async ORM (`aiosqlite`). The reason is practical: MySQL and PostgreSQL require a managed database server, which is not available on free-tier deployment infrastructure. Since SQLAlchemy abstracts the database layer, switching to MySQL or PostgreSQL requires changing exactly one line — the `DATABASE_URL` connection string in `models/database.py`. The schema, queries, and ORM models are fully compatible with both.
...
| Tool | Description |
|---|---|
| `log_interaction` | Logs a new HCP meeting — looks up HCP by name, generates AI follow-up suggestions via LLM |
...
- "Edit interaction 1 — change sentiment to Neutral"
